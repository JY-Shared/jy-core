...
gulp.task('linker', function(done){
    ...
    return gulp.src('app/_index.html')
        ...
        .pipe(linker({
            scripts:['app/scripts/app.js','app/scripts/*.js','app/scripts/*/*.js','app/scripts/*/*/*.js','!app/scripts/*/*.spec.js','!app/scripts/*/*/*.spec.js','app/event/**/*.js'],
            startTag: '<!--app js-->',
            endTag: '<!--app js end-->',
            fileTmpl: '<script src="/%s" type="text/javascript"></script>',
            appRoot: 'app/'
...
gulp.task('clinker', function (done) {
    ...
    return gulp.src(['app/less/**']).pipe(gCssample(null, tpl))
        .pipe(linker({
